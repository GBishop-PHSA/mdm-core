...
class DataClassServiceSpec extends CatalogueItemServiceSpec implements ServiceUnitTest<DataClassService> {
    ...
    void 'test copying very simple DataClass'() {
        when:
        DataClass vsimple = new DataClass(label: 'vsimple', createdBy: StandardEmailAddress.UNIT_TEST)
        dataModel.addToDataClasses(vsimple)
        DataModel copyModel = new DataModel(label: 'copy', createdBy: StandardEmailAddress.UNIT_TEST, folder: testFolder, authority: testAuthority)
    ...
    void 'test copying simple DataClass with simple data elements'() {
        given:
        DataClass content = new DataClass(createdBy: StandardEmailAddress.UNIT_TEST, label: 'content', minMultiplicity: 1, maxMultiplicity: 1)
        content.addToDataClasses(label: 'vsimple', createdBy: StandardEmailAddress.UNIT_TEST, minMultiplicity: 0, maxMultiplicity: 1)
        ...
        when:
        copyModel.addToDataTypes(new PrimitiveType(createdBy: StandardEmailAddress.UNIT_TEST, label: 'string'))
        copyModel.addToDataTypes(new PrimitiveType(createdBy: StandardEmailAddress.UNIT_TEST, label: 'integer'))
